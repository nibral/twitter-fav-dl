...
/*
    ...
    ユーザのアクセストークンを取得して、ユーザのインスタンスを返す
...
module.exports.OAuthCallback = (oauthToken, oauthCallbackQuery) => {
    return new Promise((resolve, reject) => {
        ...
        }).then((userAcccessToken) => {
            const client = new Twitter({
                consumer_key: CONSUMER_KEY,
                consumer_secret: CONSUMER_SECRET,
                access_token_key: userAcccessToken.oauth_token,
                access_token_secret: userAcccessToken.oauth_token_secret
            });
            resolve({
                userAccessToken: userAcccessToken,
                client: client
            });
        }).catch((error) => {
            reject(error);
